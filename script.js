...
async function loadPlaylist() {
  ...
  if (!API_KEY || !PLAYLIST_ID) {
    statusEl.textContent =
      "No API key / playlist configured. Edit `config.js` to add API_KEY and PLAYLIST_ID.";
  ...
  try {
    ...
    console.error(err);
    statusEl.textContent =
      "Failed to load playlist items. See console for details. Falling back to embedded playlist.";
    embedPlayer.src = `https://www.youtube.com/embed?listType=playlist&list=${PLAYLIST_ID}`;
  }
}
...
async function fetchAllPlaylistItems(apiKey, playlistId) {
  ...
  return items.filter(
    (i) =>
      i && i.snippet && i.snippet.resourceId && i.snippet.resourceId.videoId
  );
}
